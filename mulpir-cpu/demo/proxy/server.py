"""
Flask proxy server for the MulPIR-CPU PIR map demo.

Forwards API requests to the Rust mulpir-cpu server over HTTP,
...
def create_app(spiral_host: str, spiral_port: int, tiles_dir: str) -> Flask:
    ...
    # ------------------------------------------------------------------ #
    # MulPIR CPU setup endpoint
    # ------------------------------------------------------------------ #

    @app.route("/api/setup", methods=["POST"])
    def setup():
        payload = request.get_data()
        logger.info("Forwarding /api/setup (%d bytes) to mulpir-cpu server", len(payload))
        try:
            resp = requests.post(
            ...
            logger.error("MulPIR-CPU server setup failed: %s", exc)
            return jsonify({"status": "error", "message": str(exc)}), 502

    # ------------------------------------------------------------------ #
    # MulPIR CPU batch query endpoint
    # ------------------------------------------------------------------ #
    ...
    def batch_query():
        payload = request.get_data()
        logger.info(
            "Forwarding /api/batch-query (%d bytes) to mulpir-cpu server", len(payload)
        )
        try:
            resp = requests.post(
                f"{spiral_base}/api/batch-query",
                data=payload,
                headers={"Content-Type": "application/octet-stream"},
                timeout=120,
            )
            ...
            logger.error("MulPIR-CPU server batch-query failed: %s", exc)
            return jsonify({"status": "error", "message": str(exc)}), 502

    # ------------------------------------------------------------------ #
    # PIR parameters — forwarded from MulPIR-CPU server
    # ------------------------------------------------------------------ #
    ...
    def params():
        try:
            ...
            logger.error("Failed to fetch params from mulpir-cpu server: %s", exc)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Flask proxy server for the MulPIR-CPU PIR map demo"
    )
    parser.add_argument(
        "--spiral-host",
        default="localhost",
        help="Hostname of the MulPIR-CPU server (default: localhost)",
    )
    parser.add_argument(
        "--spiral-port",
        type=int,
        default=8081,
        help="Port of the MulPIR-CPU server (default: 8081)",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=8003,
        help="Port for this HTTP proxy server (default: 8003)",
    )
    parser.add_argument(
        "--tiles-dir",
        required=True,
...
def main():
    ...
    logger.info(
        "Starting MulPIR-CPU proxy on :%d  (MulPIR-CPU server at %s:%d, tiles: %s)",
        args.port,
        args.spiral_host,
        args.spiral_port,
        tiles_dir,
